...
#             print("$$$$$$$$$$$$$$$$$$$$$$",_x.shape)  #torch.Size([4, 256, 20, 20])
#             x_list.append(_x)
...
class SegFormerHead(nn.Module):
    def __init__(self, in_channels=128, embedding_dim=256, num_classes=20, index=11, **kwargs):
        ...
        self.indexes = index  # 6 or 11

        c1_in_channels, c2_in_channels, c3_in_channels, c4_in_channels = self.in_channels

        linear_layers = [MLP(input_dim=c1_in_channels, embed_dim=embedding_dim) for _ in range(self.indexes)]
        self.linears_modulelist = nn.ModuleList(linear_layers)

        # Space attention layers
        self.spatial_attention = nn.Sequential(
            nn.Conv2d(embedding_dim * self.indexes, 1, kernel_size=1),
            nn.Sigmoid()
        )
        self.linear_fuse = nn.Conv2d(embedding_dim * self.indexes, embedding_dim, kernel_size=1)
        self.dropout = nn.Dropout2d(0.1)
    ...
    def forward(self, x_all):
        ...
        for ind in range(x_all.shape[0]):
            x = x_all[ind, :, :, :, :]
            n, _, h, w = x.shape
            _x = self.linears_modulelist[ind](x.float()).permute(0, 2, 1).reshape(n, -1, h, w)
            x_list.append(_x)
        ...
        attention_map = self.spatial_attention(x_list)
        x = x_list * attention_map  # Apply attention weights
