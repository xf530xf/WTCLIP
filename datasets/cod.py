...
def load_cls_label_list(name_list_dir):
    ...
    return np.load(os.path.join(name_list_dir,'image_labels.npy'), allow_pickle=True).item()
...
class VOC12SegDataset(VOC12Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image = Resize((320, 320), interpolation=BICUBIC)(image)
        # image = Resize((320, 320), interpolation=BICUBIC)(Image.fromarray(image))
